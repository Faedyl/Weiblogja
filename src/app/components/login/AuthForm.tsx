...
export default function AuthForm({ mode = "login" }: AuthFormProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			if (!isLogin) {
				// Registration flow - create user account first, then send OTP
				const result = await signIn("credentials", {
					...
					isRegistering: "true",  // Register the user
					redirect: false,
				});
				...
				if (result?.error) {
					...
					try {
						await sendOTP(formData.email, formData.name || formData.email.split('@')[0]);
	...
	const handleOTPVerification = async (e: React.FormEvent) => {
		...
		try {
			...
			if (result.success) {
				// OTP verified successfully, now attempt to log in the user
				// The user should now be verified and able to log in
				const loginResult = await signIn("credentials", {
					...
					department: formData.department,
					orcid: formData.orcid,
					isRegistering: "false", // Now we're logging in, not registering
					redirect: false,
				});
						...
						<form onSubmit={handleOTPVerification} className={styles.form}>
							...
							<div className={styles.resendContainer}>
								<button
									...
									onClick={async () => {
										try {
											await sendOTP(otpEmail, formData.name || otpEmail.split('@')[0]);
							...
							<div className={styles.backToRegistration}>
								<button
									type="button"
